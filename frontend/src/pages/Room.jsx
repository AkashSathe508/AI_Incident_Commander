...
export default function Room() {
  ...
  // ── Participant state ─────────────────────────────────────────────────────
  // { uid: number, name: string, isSelf: boolean }
  ...
  // ── Agora refs (not state — never trigger re-renders) ─────────────────────
  ...
  async function joinRoom(e) {
    ...
    try {
      // 1. Obtain Agora credentials from backend (token stays server-side)
      ...
      // 3. Wire remote-user events BEFORE joining the channel
      ...
      // 4. Speaking indicator (fires every 2s for active speakers)
      ...
      // 6. Capture and publish the local microphone
      ...
      setPhase("prompt");
      // Clean up any partial Agora state
      micTrackRef.current?.close();
